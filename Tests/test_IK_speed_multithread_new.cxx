...
void producerThread(OpenSimLive::IMUHandler& genericDataReader, VariableManager& vm) {
	do {
		//std::cout << "Buongiorno sono il producer" << std::endl;
		// get time stamp
		...
		//std::cout << vm.maxBufferSize << std::endl;
...
void consumerThread(VariableManager& vm, OpenSimLive::IMUInverseKinematicsToolLive& IKTool, OpenSimLive::ThreadPoolContainer& threadPoolContainer) {
	...
	do {
		//std::cout << vm.orientationBuffer.size() << std::endl;
		//std::cout  << vm.bufferInUse << std::endl;

		// if producer is not accessing the buffer and the buffer contains values
		if (!vm.bufferInUse && vm.orientationBuffer.size() > 0) {
			//std::cout << "Buongiorno sono il consumer" << std::endl;
			vm.bufferInUse = true;
			std::lock_guard<std::mutex> lock(mainMutex);
...
void ConnectToDataStream(double inputSeconds, int inputThreads) {
	...
	manufacturer = OpenSimLive::IMUHandler::IMUType::mpu9250;
	//manufacturer = OpenSimLive::IMUHandler::IMUType::simulated;
	std::cout << (int)manufacturer << std::endl;
	...
	// CALIBRATION STEP
	if (manufacturer == OpenSimLive::IMUHandler::IMUType::mpu9250) {
		genericDataReader.updateFirstQuaternionTable();
	}
	else genericDataReader.updateQuaternionTable();
	...
	//genericDataReader.updateQuaternionTable();
	//auto quatTable = genericDataReader.getQuaternionTable();
