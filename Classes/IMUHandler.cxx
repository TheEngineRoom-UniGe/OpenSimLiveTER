...
void IMUHandler::initialize() {
	...
	if (IMUType_ == IMUType::mpu9250) {
		std::cout << "I AM USING MPU9250 AS IMUTYPE" << std::endl;
		...
		mpuObject_.reset(new OpenSimLive::MpuDataReader);
		mpuObject_->setSaveQuaternions(saveQuaternions);
		unsigned int dataReaderResult = 0;
		dataReaderResult = mpuObject_->InitiateStartupPhase();
		mpuObject_->PrintLabels();
		mpuObject_->ResizeQuatVector();
		//simulatedObject_->setIsRandom(isRandom);
		// initially generate a set of identity quaternions for uniform calibration
		//mpuObject_->generateIdentityQuaternions();
		mpuObject_->generateCalibrationQuaternion();
		std::cout << "quaternion generated" << std::endl;
		quaternionTimeSeriesTable_ = mpuObject_->getTimeSeriesTable();
		std::cout << "retrieved timeseries table" << std::endl;

		std::cout << quaternionTimeSeriesTable_ << std::endl;
	}
	else if (IMUType_ == IMUType::xsens) {
...
void IMUHandler::updateFirstQuaternionTable() {
	std::cout << "First quaternion table MPU9250 ____ will be use to calibrate the model" << std::endl;
	quaternionTimeSeriesTable_ = mpuObject_->getTimeSeriesTable();
}
...
void IMUHandler::updateQuaternionTable() {
	...
	else if (IMUType_ == IMUType::mpu9250) {
		//quaternionTimeSeriesTable_ = mpuObject_->getTimeSeriesTable();
		quaternionTimeSeriesTable_ = mpuObject_->generateNewQuaternion();
	}
	else if (IMUType_ == IMUType::delsys) {
